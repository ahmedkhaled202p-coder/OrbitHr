...
# ==========================================================================
# Orbit HR v4.9.0 final readiness patch
# - Complete server CRUD for notifications and custom internal pages.
# - Fix fixed service-center route permissions (organization/approvals/selfservice).
# - Accept manual attendance HH:mm values through the production collection API.
# - Keep /api/health and exported API metadata aligned with the final version.
# ==========================================================================
APP_VERSION = "4.9.0-final-ready"

_prev_parse_local_dt_v49 = parse_local_dt
def parse_local_dt(value: Any, state: Dict[str, Any], default_date: Optional[_dt.date] = None) -> Optional[_dt.datetime]:  # type: ignore[override]
    if not value:
        return None
    s = str(value).strip()
    if default_date and ("T" not in s and " " not in s):
        t = parse_time_hm(s)
        if t:
            return _dt.datetime.combine(default_date, t, tzinfo=company_timezone(state))
    return _prev_parse_local_dt_v49(value, state, default_date)

# Complete role permissions for the final service set.
ROLE_PERMISSIONS.setdefault("manager", set()).update({
    "notifications:read", "notifications:write", "customPages:read", "customPages:write",
    "announcements:read", "announcements:write", "approvals:read", "approvals:write", "organization:read", "selfservice:read",
})
ROLE_PERMISSIONS.setdefault("hr", set()).update({
    "notifications:read", "notifications:write", "customPages:read", "customPages:write",
    "announcements:read", "announcements:write", "approvals:read", "approvals:write", "organization:read", "selfservice:read",
})
ROLE_PERMISSIONS.setdefault("finance", set()).update({"notifications:read", "announcements:read", "selfservice:read"})
ROLE_PERMISSIONS.setdefault("employee", set()).update({
    "notifications:read", "customPages:read", "announcements:read", "selfservice:read",
})

# Add the two missing UI modules to the granular record API.
COLLECTION_API_RULES.update({
    "notifications": {"perm": "notifications:write", "read_perm": "notifications:read"},
    "customPages": {"perm": "customPages:write", "read_perm": "customPages:read"},
})
STATUS_PERMISSIONS.update({
    "notifications": "notifications:write",
    "customPages": "customPages:write",
})
ALLOWED_COLLECTION_KEYS = set(COLLECTION_API_RULES)
SAFE_STATUSES.update({"archived", "unread", "read"})

_prev_collection_visible_items_v49 = _collection_visible_items

def _collection_visible_items(state: Dict[str, Any], actor: sqlite3.Row, key: str) -> List[Dict[str, Any]]:  # type: ignore[override]
    items = _prev_collection_visible_items_v49(state, actor, key)
    role = lower(actor["role"])
    emp_id = clean_str(actor["employee_id"])
    if key == "notifications" and not (is_admin(actor) or role in ("manager", "hr")):
        employee = emp_by_id(state, emp_id) if emp_id else None
        branch = clean_str(employee.get("branch")) if employee else ""
        filtered = []
        for x in items:
            audience = clean_str(x.get("audience") or x.get("target") or "all")
            if clean_str(x.get("status")) not in ("sent", "published", "active", ""):
                continue
            if audience in ("all", "employees", ""):
                filtered.append(x)
            elif audience == "branch" and (not x.get("branch") or clean_str(x.get("branch")) == branch):
                filtered.append(x)
            elif clean_str(x.get("userId")) in (actor["id"], emp_id):
                filtered.append(x)
        return filtered
    if key == "customPages" and not (is_admin(actor) or role in ("manager", "hr")):
        return [x for x in items if clean_str(x.get("status")) in ("published", "active") and clean_str(x.get("visibility") or "all") in ("all", "employees", "")]
    return items


def validate_attendance_save(state: Dict[str, Any], rec: Dict[str, Any], current_id: str = "") -> List[str]:  # type: ignore[override]
    """Final validator accepts ISO datetimes and simple HH:mm values tied to rec.date."""
    errors: List[str] = []
    if not rec.get("employeeId") or not emp_by_id(state, clean_str(rec.get("employeeId"))):
        errors.append("الموظف غير موجود")
    ...
    if not d:
        errors.append("تاريخ الحضور غير صحيح")
    ci = parse_local_dt(rec.get("checkIn"), state, d) if d else parse_dt(rec.get("checkIn"))
    co = parse_local_dt(rec.get("checkOut"), state, d) if d else parse_dt(rec.get("checkOut"))
    if co and not ci:
        errors.append("لا يمكن تسجيل الانصراف قبل تسجيل الحضور")
    if ci and co and co < ci:
        errors.append("وقت الانصراف قبل وقت الحضور")
...
_prev_api_reports_summary_v49 = _api_reports_summary_v44

def _api_reports_summary_v49(self: "OrbitHandler", qs: Dict[str, List[str]]) -> None:
    # Keep the v4.8 report behavior, but expose final version metadata through health.
    return _prev_api_reports_summary_v49(self, qs)

_api_reports_summary_v44 = _api_reports_summary_v49

# Make health report the completed collection list and schema version.
def _api_health_v49(self: "OrbitHandler") -> None:
    actor = current_user(self)
    if not actor:
        return self.send_json({"ok": True, "status": "online", "version": APP_VERSION, "serverTime": local_now_iso()})
    role = lower(actor["role"])
    ...
    with _db_lock, db() as con:
        state, updated_at = load_state_from_db(con)
        keys = ["employees", "users", "attendance", "payroll", "leaves", "missions", "expenses", "adjustments", "custodies", "advances", "dues", "documents", "notifications", "customPages", "announcements"]
        counts = {k: len(as_list(state.get(k))) for k in keys}
        counts.update({
            "structuredEmployees": con.execute("SELECT COUNT(*) c FROM hr_employees").fetchone()["c"],
            "structuredAttendance": con.execute("SELECT COUNT(*) c FROM hr_attendance").fetchone()["c"],
    ...
    self.send_json({
        ...
        "productionCandidate": False,
        "productionReady": True,
        "apis": {"collections": sorted(ALLOWED_COLLECTION_KEYS), "leaveBalance": True, "reportsSummary": True, "settings": True},
    })

OrbitHandler.api_health = _api_health_v49

_init_db_before_v49 = init_db

def init_db() -> None:  # type: ignore[override]
    _init_db_before_v49()
    with _db_lock, db() as con:
        con.execute("CREATE TABLE IF NOT EXISTS system_meta(key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT NOT NULL)")
        con.execute("INSERT OR REPLACE INTO system_meta(key,value,updated_at) VALUES('schema_version','4.9.0',?)", (utc_now(),))
