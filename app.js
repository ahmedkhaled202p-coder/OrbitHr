...
let APP_VERSION = '4.9.0-final-ready';
...
/* =========================================================
   Orbit HR v4.9 final readiness client patch
   - Fixed service-center permissions for organization / approvals / self-service.
   - Saves generic modules through granular collection APIs, including notifications and custom pages.
   - Keeps manual attendance compatible with HH:mm values by using the server collection API.
   ========================================================= */
APP_VERSION='4.9.0-final-ready';

const _canAccessRouteBeforeV49 = canAccessRoute;
canAccessRoute = function(route, user=currentUser){
  if(!user)return false;
  if(user.role==='admin')return true;
  if(userHasCustomPermissions(user))return userPermissions(user).includes(route);
  if(typeof FIXED_SERVICE_ROLES!=='undefined' && FIXED_SERVICE_ROLES[route]?.includes(user.role))return true;
  if(typeof EXTRA_MODULES!=='undefined' && EXTRA_MODULES[route]?.roles?.includes(user.role))return true;
  return _canAccessRouteBeforeV49(route,user);
};

serviceAllowed = function(route){return canAccessRoute(route)};

const _professionalHealthChecksBeforeV49 = professionalHealthChecks;
professionalHealthChecks=function(){
  const issues=_professionalHealthChecksBeforeV49().filter(i=>{
    const t=String(i?.title||'');
    return !t.includes('الإخطارات') && !t.includes('مصمم الصفحات');
  });
  const missing=[];
  ['notifications','customPages'].forEach(k=>{if(!Array.isArray(state[k]))missing.push(k)});
  if(missing.length)addReadinessIssue(issues,'error','الخدمات','بعض الخدمات غير مهيأة في البيانات','الإخطارات ومصمم الصفحات يحتاجان تهيئة أولية.','أعد تحميل بيانات السيرفر أو استرجع نسخة حديثة');
  return issues;
};

function moduleFormRecord(route,id=''){
  const cfg=EXTRA_MODULES[route],form=$('#moduleForm');
  if(!cfg||!form)return null;
  if(!form.reportValidity())return null;
  const o=Object.fromEntries(new FormData(form).entries());
  cfg.fields.filter(f=>f.type==='number').forEach(f=>o[f.name]=Number(o[f.name]||0));
  if(!moduleCanManage(cfg))o.status=cfg.defaultStatus||'pending';
  if(cfg.employeeField&&currentUser.role==='employee'&&!canManage())o[cfg.employeeField]=ownEmployee()?.id||'';
  if(o.from&&o.to&&String(o.to)<String(o.from)&&!o.from.includes(':')){toast('تاريخ النهاية يجب ألا يسبق البداية','error');return null;}
  const existing=id?state[cfg.stateKey].find(x=>x.id===id):null;
  const record=existing?{...existing,...o,updatedAt:new Date().toISOString()}:{id:uid(route.slice(0,3)),...o,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),createdBy:currentUser?.id||''};
  return record;
}

const _saveModuleRecordLocalV49 = saveModuleRecord;
saveModuleRecord = async function(route,id){
  const cfg=EXTRA_MODULES[route];
  const record=moduleFormRecord(route,id);
  if(!cfg||!record)return;
  applyModuleSideEffects(route,record);
  if(!orbitToken())return _saveModuleRecordLocalV49(route,id);
  try{
    const r=await orbitCollectionSave(route,record,id||'');
    if(r.item)record.id=r.item.id||record.id;
    // Persist complaint side-effect adjustment when created.
    if(route==='complaints'&&record.linkedAdjustmentId){
      const adj=state.adjustments.find(x=>x.id===record.linkedAdjustmentId);
      if(adj)await orbitCollectionSave('adjustments',adj,adj.id);
    }
    closeModal();render();toast('تم حفظ السجل على السيرفر');
  }catch(err){
    toast(err?.payload?.message||err?.payload?.error||'تعذر حفظ السجل على السيرفر','error');
  }
};

const _setModuleStatusLocalV49 = setModuleStatus;
setModuleStatus = async function(route,id,status){
  const cfg=EXTRA_MODULES[route],r=state[cfg?.stateKey]?.find(x=>x.id===id);
  if(!cfg||!r)return;
  const next={...r,status,updatedAt:new Date().toISOString(),statusUpdatedBy:currentUser?.id||'',statusUpdatedAt:new Date().toISOString()};
  applyModuleSideEffects(route,next);
  if(!orbitToken())return _setModuleStatusLocalV49(route,id,status);
  try{
    const res=await orbitCollectionStatus(route,id,status);
    if(res.item)orbitMergeItem(cfg.stateKey,res.item);
    if(route==='complaints'&&next.linkedAdjustmentId){
      const adj=state.adjustments.find(x=>x.id===next.linkedAdjustmentId);
      if(adj)await orbitCollectionSave('adjustments',adj,adj.id);
    }
    render();toast(`تم تحديث الحالة إلى: ${statusText(status)}`);
  }catch(err){toast(err?.payload?.message||err?.payload?.error||'تعذر تحديث الحالة','error')}
};

const _deleteModuleRecordLocalV49 = deleteModuleRecord;
deleteModuleRecord = async function(route,id){
  const cfg=EXTRA_MODULES[route];
  if(!cfg)return;
  if(!confirm(`حذف السجل من ${cfg.label}؟`))return;
  if(!orbitToken())return _deleteModuleRecordLocalV49(route,id);
  try{
    const r=state[cfg.stateKey].find(x=>x.id===id);
    await orbitCollectionDelete(route,id);
    if(route==='complaints'&&r?.linkedAdjustmentId){
      try{await orbitCollectionDelete('adjustments',r.linkedAdjustmentId)}catch{}
      state.adjustments=state.adjustments.filter(x=>x.id!==r.linkedAdjustmentId);
    }
    render();toast('تم حذف السجل من السيرفر');
  }catch(err){toast(err?.payload?.message||err?.payload?.error||'تعذر حذف السجل','error')}
};

const _syncBiometricDeviceLocalV49 = syncBiometricDevice;
syncBiometricDevice = async function(id){
  const r=state.biometricDevices.find(x=>x.id===id);if(!r)return;
  r.lastSync=new Date().toISOString();r.status='connected';r.updatedAt=new Date().toISOString();
  if(!orbitToken())return _syncBiometricDeviceLocalV49(id);
  try{await orbitCollectionSave('biometricDevices',r,id);render();toast('تم حفظ حالة الجهاز على السيرفر')}
  catch(err){toast(err?.payload?.message||'تعذر حفظ حالة الجهاز','error')}
};
